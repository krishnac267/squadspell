...
function processLine(line) {
  ...
  if (/^\(duplicate\)/i.test(t)) return null;
  if (/^[A-Za-z]\s*\(continued\)$/i.test(t)) return null;
...
if (files.length === 0) {
  console.error('Usage: node normalize-part8-append.cjs <file.txt> [...]');
  process.exit(1);
}

let out = '\n# Part 8 – longer words A–Z (space-separated batch)\n';
